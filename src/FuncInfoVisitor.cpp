...
std::pair<int, int> FuncInfoVisitor::HandleOtherCompounds(const Stmt* body, int depth)
{
    ...
    for (; it != body->child_end(); ++it) {
        ...
        bool advance = !(it->getStmtClass() == Stmt::CompoundStmtClass && (stmtClass == Stmt::WhileStmtClass || stmtClass == Stmt::CXXTryStmtClass || stmtClass == Stmt::ForStmtClass || stmtClass == Stmt::SwitchStmtClass || stmtClass == Stmt::CXXForRangeStmtClass || stmtClass == Stmt::CXXCatchStmtClass || stmtClass == Stmt::CaseStmtClass || stmtClass == Stmt::DefaultStmtClass));
...
std::pair<int, int> FuncInfoVisitor::StmtCount(const Stmt* body, int depth)
{
    ...
    auto stmtClass = body->getStmtClass();
    bool is_compound = std::find(compoundStatements.begin(), compoundStatements.end(), stmtClass) != compoundStatements.end();
